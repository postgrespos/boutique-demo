...
var (
	grpcRequestsTotal = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "shippingservice_grpc_requests_total",
		Help: "Total number of gRPC requests processed, by method and status code.",
	}, []string{"method", "code"})

	grpcRequestDuration = promauto.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "shippingservice_grpc_request_duration_seconds",
		Help:    "Latency of gRPC requests in seconds, by method.",
		Buckets: prometheus.DefBuckets,
	}, []string{"method"})

	quotedCostUSD = promauto.NewHistogram(prometheus.HistogramOpts{
		Name:    "shippingservice_quote_cost_usd",
		Help:    "Shipping cost (in USD major units) returned by GetQuote.",
		Buckets: []float64{1, 5, 10, 20, 50, 100},
	})

	shipmentsTotal = promauto.NewCounter(prometheus.CounterOpts{
		Name: "shippingservice_shipments_total",
		Help: "Total number of shipments created via ShipOrder.",
	})
)
